...
void IGameController::Tick()
{
	...
	if(g_Config.m_SvInactiveKickTime > 0)
	{
		...
		#endif
                        if(GameServer()->m_apPlayers[i] && GameServer()->m_apPlayers[i]->GetTeam() != TEAM_SPECTATORS && Server()->GetAuthedState(i) == AUTHED_NO && !GameServer()->m_apPlayers[i]->m_IsDummy) // iDDNet
			{
